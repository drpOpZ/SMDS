...
The output of the second commands suggests there are no missing values in the table.

```{r daag4.2, echo=TRUE}
# 2.
#make a table to show #of males and females for each ifferent sport
...
= e^{\lambda (e^{s} - 1)}
$$
2. Since $Y_1 \sim \text{Pois}(\lambda_1)$ and $Y_2 \sim \text{Pois}(\lambda_2)$ are independent we know that
$$
M_{Y_1+Y_2}(s) = M_{Y_1}(s)M_{Y_2}(s) 
= e^{\lambda_1 (e^{s} - 1)}e^{\lambda_2 (e^{s} - 1)}
= e^{(\lambda_1+\lambda_2) (e^{s} - 1)}\ ,
$$
which coincides with the m.g.f. of a Poisson distribution with parameter $\lambda = \lambda_1 + \lambda_2$ in an arbitrarily small interval enclosing 0.

3. It follows from the previous result that, given $n$ indipendent random variables $X_i ~ \text{Pois}(\lambda_i)$, we have
$$
...
$$
therefore if we assume $\lambda_i = \lambda\ \forall i$, the CLT also tells us that, as $n$ goes to infinity the distribution of $Z$ coverges to a normal distribution with $\mu = n\lambda$ and $\sigma^2 = n\lambda$, which combined with our previous findings gives us that
...
lambda <- 5
for(lambda in c(1,5,10,100)){
  
  plot(0:125, dpois(0:125,lambda),
       xlim=c(0,125), ylim=c(0,0.20),
       col="red", type="l",
       xlab="x", ylab="f(x)",
       main=paste("CLT for Poisson distribution with lambda= ",lambda),
       cex.main=0.6)
  lines(0:125, dnorm(0:125,lambda,sqrt(lambda)),
        col="black", type="h")
```

<!--NOTE: Plotting anything at this point apparently breaks the layout!-->

<!--```{r cs1.9, echo=FALSE, fig.cap="The plots show that the normal curve approximation (black lines) of the poisson distribution (red curve) gets better as lambda increases."}
