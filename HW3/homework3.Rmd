...
## Ex. 1
Given the following samples $y_1,...,y_15$ of a R.V. $Y \sim \text{Weibull}(\gamma,\beta)$
```R
...
In the case in which $\gamma$ was the parameter of interest I was able, equating to 0 the derivative of the log likelihood wrt $\beta$, to find an analytical formula to compute $\beta$ as function of $\gamma$.
  ...
  But now that $\beta$ is the parameter of interest I cannot find a close formula to compute $\gamma$ analytically having $\beta$ from the partial derivative of the log likelihood wrt $\gamma$. Hence I've created a function $\textit{deriv()}$ which computes the derivative $\frac{\delta}{\delta\gamma}l(\gamma,\beta,y)$ and another function $gamma.beta_fun$ which given a value for $\beta$ computes numerically a zero of the $\textit{deriv()}$ function which now depends only on $\gamma$.
...
In $\textit{sim}$ in the code above, you find the MCMC output which allows to approximate the posterior distribution of our parameter of interest with $S$ draws of $\theta$. Please, produce an histogram for these random draws $\theta(1),…,\theta(S)$, compute the empirical quantiles, and overlap the true posterior distribution.
...
Now I can use $\textit{stan}$ to simulate values from the posterior distribution:

```{r lab5.2, echo=TRUE}
data<- list(N=n, y=y, sigma =sqrt(sigma2), mu = mu, tau = sqrt(tau2))
fit <- stan(file="/home/claudia/Units/StatisticalMethodsforDataScience/Exercises/normal.stan", data = data, chains = 4, iter=2000)
...
curve(dnorm(x, mu_star, sd_star),
      xlim=c(.9,4.6),
      xlab=expression(theta), ylab="posterior", col="blue", lwd=2,
      cex.lab=2)  
```
...
```
The Beta distribution is not suitable for our problem, the main reason is the fact that it is defined only for $x\in [0,1]$ so the maximum length of the phone calls we receive can only be 1 minute, which is very unlikely to be real.
...
```
The normal distribution is not suitable for our problem, since it makes no sense that there is a probability different than zero of a call having length = 0 minutes.
...
```
The gamma distribution is a suitable solution for our problem since it has zero probability of having zero length calls and the major part of the calls will be distributed around one and a half minutes, which makes sense.
...
Follow the instructions to download and install the `rstan` library. Once you did it successfully, open the file model called "biparametric.stan", and replace the line:   `target+=cauchy_lpdf(sigma|0,2.5);`
...
Instead of using a half Cauchy distribution we are using a uniform distribution between 0.1 and 10. Both the trace plots converge but the values are slighlty different than before. 


## Ex. 9

Reproduce the first plot above for the soccer goals, but this time by replacing $\textit{prior 1}$ with a $\textit{Gamma(2,4)}$. Then, compute the final Bayes factor matrix $\textit{BF_matrix}$ with this new prior and the other ones unchanged, and comment. Is still $\textit{prior 2}$ favorable over all the others?
...
From the Bayes factor matrix I can compare different priors. Of course just the first row and the first column change because I've changed just the first prior. I can see that the second prior is still the preferable one because it's still favored over Prior3 and Prior4 but it's also favored over new prior1 because $BF[2,1]=6.38$ says that Prior2 is 6.38 times more preferable than Prior1. Prior3 is still the less preferable one.
...
#adesso per ogni bootstrap devo:
# 1. calcolare psi e metterlo in psi_vect
# 2.calcolare una stima di SE(psi*) con jackknife - QUESTO NON LO DEVO PIU FARE
# 3. calcolare zeta come (psi[i] - psi_obs)/se_jack
for(i in 1:B) {
  # estraggo un campione con ripetizione degli indici e chiamerÃ² la 
  # funzione psi_func su queste righe del dataset
  ind <- sample(1:n, n, replace = TRUE)
  # 1. in psi ci metto il valore osservato per questo campione
  psi_vect[i,] <- psi_fun(score,ind)
  zeta_vect[i] <- (psi_vect[i,1]-psi_obs[1])/psi_vect[i,2]
}

#calcolo i quantili delle psi:  
zeta <- quantile(zeta_vect, prob=c(0.975, 0.025))

#let's compute the confidence interval

# qui se(psi) no è quello del jackknife ma una stima ottenuta calcolando
# lo standard error della prima colonna di psi_vect
SE <- sd(psi_vect[,1])
...
```{r lec2, echo=TRUE}
# la funzione Ã¨ giÃ  modificata con l'indice che serve
a <- boot(score,psi_fun,R=10000)
#si  puÃ² vedere in a$t che nella seconda colonna c'Ã¨ la stima di se
# per ogni bootstrap, che serve per usare boot.ci con type="stud"
a$t
#costruisco l'intervallo
